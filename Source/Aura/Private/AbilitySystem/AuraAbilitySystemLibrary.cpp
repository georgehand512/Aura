...
// storing the dataclass for the game
void UAuraAbilitySystemLibrary::InitializeDefaultAttributes(const UObject* WorldContextObject, ECharacterClass CharacterClass, float Level, UAbilitySystemComponent* ASC)
{
	// get and set a source object to apply to
	AActor* AvatarActor = ASC->GetAvatarActor();

	// now we have gamemode we can access the data asset -> Charater Class Info
	UCharacterClassInfo* CharacterClassInfo = GetCharacterClassInfo(WorldContextObject);
	FCharacterClassDefaultInfo ClassDefaultInfo = CharacterClassInfo->GetClassDefaultInfo(CharacterClass);

	// apply the gameplay effects from that data to an ability system component (start with bottom call and assembly all necessary data to pass in on lines above)
	FGameplayEffectContextHandle PrimaryAttributesContextHandle = ASC->MakeEffectContext();
	PrimaryAttributesContextHandle.AddSourceObject(AvatarActor);
	const FGameplayEffectSpecHandle PrimaryAttributesSpecHandle = ASC->MakeOutgoingSpec(ClassDefaultInfo.PrimaryAttributes, Level, PrimaryAttributesContextHandle);
	ASC->ApplyGameplayEffectSpecToSelf(*PrimaryAttributesSpecHandle.Data.Get());

	// applly secondary effects
	FGameplayEffectContextHandle SecondaryAttributesContextHandle = ASC->MakeEffectContext();
	SecondaryAttributesContextHandle.AddSourceObject(AvatarActor);
	const FGameplayEffectSpecHandle SecondaryAttributesSpecHandle = ASC->MakeOutgoingSpec(CharacterClassInfo->SecondaryAttributes, Level, SecondaryAttributesContextHandle);
	ASC->ApplyGameplayEffectSpecToSelf(*SecondaryAttributesSpecHandle.Data.Get());

	// applly vital effects
...
{
	// now we have gamemode we can access the data asset -> Charater Class Info ()only on server so return early if not valid
	UCharacterClassInfo* CharacterClassInfo = GetCharacterClassInfo(WorldContextObject);
	if (CharacterClassInfo == nullptr) return;

	// loop through TArray of common abilities
	...
	// loop through startup abilities
	const FCharacterClassDefaultInfo& DefaultInfo = CharacterClassInfo->GetClassDefaultInfo(CharacterClass);
	for (TSubclassOf<UGameplayAbility> AbilityClass: DefaultInfo.StartupAbilities)
	{
		// get player level using combat interface on ASC
		if (ASC->GetAvatarActor()->Implements<UCombatInterface>())
		{
			// make ability spec with player level passed in
...
{
	// need character class data asset, which exists on the gamemode
	const AAuraGameModeBase* AuraGameMode = Cast<AAuraGameModeBase>(UGameplayStatics::GetGameMode(WorldContextObject));
	if (AuraGameMode == nullptr) return nullptr;

	// now we have gamemode
...
// Exposing GETTERS custom Effect Context to Blueprints
bool UAuraAbilitySystemLibrary::IsBlockedHit(const FGameplayEffectContextHandle& EffectContextHandle)
{
	//cast to custom Aura Context ... must be static cast using standard C cast requiring * within <> and be const
...
// Get alive players within radius(modified from similar existing Engine function function ApplyRadialDamageWithFalloff )
	                                                       ...
	                                                       const FVector& SphereOrigin)
{
	...
	if ( const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		{
			// check template function to see if implements our ICombatInterface
...
{
	//cast to custom Aura Context ... must be static cast using standard C cast requiring * within <>, this is not const as it's changed
