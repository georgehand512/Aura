...
void UAuraAbilitySystemComponent::AddCharacterAbilities(const TArray<TSubclassOf<UGameplayAbility>>& StartupAbilities)
{
	...
	{
		FGameplayAbilitySpec AbilitySpec = FGameplayAbilitySpec(AbilityClass, 1); // 1 is level replace later?
...
{
	// locking the ability list prevents it being changed whilst we are looping through it
	...
	for (const FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		...
		{
			// %hs and the __FUNCTION__ is a way to log the name of the function we are in
...
void UAuraAbilitySystemComponent::UpdateAbilityStatuses(int32 Level)
{
	...
	for (const FAuraAbilityInfo& Info : AbilityInfo->AbilityInformation)
	{
		...
		if (GetSpecFromAbilityTag(Info.AbilityTag) == nullptr)
		{
			...
			GiveAbility(AbilitySpec);
			MarkAbilitySpecDirty(AbilitySpec); // way to force ability spec to replicate imdeiately
...
// replicate UI Spell globes to clients
